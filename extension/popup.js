...
// --- NEW: Popup Generator Option Elements ---
...
popupGenLengthSlider.addEventListener('input', () => { // --- NEW ---
...
// --- Authentication --- (Keep functions as before: checkLoginStatus, handleLoginAttempt, handleLogout)
...
function hideAddForm() { addPasswordForm.classList.add('hidden'); addPasswordBtn.classList.remove('hidden'); serviceInput.value = ''; newUsernameInput.value = ''; newPasswordInput.value = ''; masterPasswordSaveInput.value = ''; hideSaveError(); /* Reset generator options on hide */ popupGenLengthSlider.value = 16; popupGenLengthValueSpan.textContent = '16'; popupGenLowercaseCheckbox.checked = true; popupGenUppercaseCheckbox.checked = true; popupGenDigitsCheckbox.checked = true; popupGenSymbolsCheckbox.checked = true; }
...
// --- Credential Handling --- (Keep functions as before: loadAndDisplayCredentials, filterAndRenderList, renderCredentialItem, handleShowDetails, handleCopyPassword, handleFillPassword, fillPasswordOnPage, handleSavePassword, handleSearchFilter)
...
async function handleShowDetails(listItem) {
    ...
    const encryptedDataB64 = listItem.dataset.encrypted; const infoDiv = listItem.querySelector('.item-info'); const usernameSpan = infoDiv.querySelector('.username-display'); const showBtn = listItem.querySelector('.item-actions button:nth-child(1)'); const copyBtn = listItem.querySelector('.item-actions button:nth-child(2)'); const fillBtn = listItem.querySelector('.item-actions button:nth-child(3)');
    if (showBtn.textContent === 'Show') {
        showBtn.textContent = '...'; showBtn.disabled = true; const decrypted = await decryptData(derivedEncryptionKey, encryptedDataB64); showBtn.disabled = false;
        if (decrypted) { infoDiv.querySelector('strong').textContent = escapeHtml(decrypted.service || '(No Service)'); usernameSpan.textContent = escapeHtml(decrypted.username || '(No Username)'); listItem.dataset.decryptedPassword = decrypted.password; showBtn.textContent = 'Hide'; copyBtn.style.display = 'inline-block'; fillBtn.style.display = 'inline-block'; usernameSpan.style.color = ''; }
        else { usernameSpan.textContent = '(Decrypt Failed)'; usernameSpan.style.color = 'red'; showBtn.textContent = 'Error'; }
    } else {
        const originalHint = allCredentials.find(c => c.encrypted_data === encryptedDataB64)?.service_hint; infoDiv.querySelector('strong').textContent = escapeHtml(originalHint || '(No Hint)'); usernameSpan.textContent = '(Username Hidden)'; usernameSpan.style.color = ''; showBtn.textContent = 'Show'; copyBtn.style.display = 'none'; fillBtn.style.display = 'none'; delete listItem.dataset.decryptedPassword;
    }
}
...
// --- Generate Password Handler (Popup) ---
async function handleGeneratePopupPassword() {
     if (!userEmail || !derivedEncryptionKey) {
         showSaveError("Please log in fully to generate passwords."); return;
     }
     hideSaveError();
     generatePopupPasswordBtn.disabled = true; generatePopupPasswordBtn.textContent = '...';

     // --- Read Options from Popup Controls ---
     const options = {
     ...
     try {
         const response = await fetch(`${API_BASE_URL}/api/generate_password`, {
             method: 'POST',
             headers: { 'Content-Type': 'application/json' },
             credentials: 'include',
             body: JSON.stringify(options) // Send selected options
         });
         const result = await response.json();

         if (response.ok && result.password) {
             newPasswordInput.value = result.password;
             newPasswordInput.type = 'text'; // Show generated password
             setTimeout(() => {
                 if (newPasswordInput.type === 'text') newPasswordInput.type = 'password';
             }, 2000);
         } else {
             throw new Error(result.error || 'Failed to generate password from API.');
         }
     } catch (error) {
         showSaveError(`Generate Error: ${error.message}`);
         console.error('Generate Password Error (Popup):', error);
     } finally {
         generatePopupPasswordBtn.disabled = false; generatePopupPasswordBtn.textContent = 'Generate';
     }
}
